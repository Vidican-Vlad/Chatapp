...
class FriendController {
  async getFriends({ auth }) {
    ...
    // }));
    const Friends1 = (await Database.select('friends.id','user_id','friend_id','status','username')
      .from('friends')
      .innerJoin('users','friends.user_id','users.id')
      .where('friend_id',user.id)
      .where('status',1)).map((friendRelation) => ({
      id: friendRelation.id,
      username: friendRelation.username,
      friend_id: friendRelation.user_id,
      status: friendRelation.status,
    }))
    const Friends2 = (await Database.select('friends.id','user_id','friend_id','status','username')
      .from('friends')
  ...
  async getPendingReq({auth, request}) {
    ...
    const Requests = await Database.select('user_id','friend_id','status','username')
      .from('friends')
      .innerJoin('users','friends.user_id','users.id')
      .where("friend_id",user.id)
      .where('status',0);
  ...
  async notAlreadyFriends(user, friend)
  {
    ...
    if(user.id === friend.id)
      return false;
    const existingFriendRelation1 = await Database.select('user_id','friend_id')
      .from('friends')
      .where('user_id',user.id)
      .where('friend_id',friend.id)
    const existingFriendRelation2 = await Database.select('user_id','friend_id')
      .from('friends')
      .where('friend_id',user.id)
      .where('user_id',friend.id)
    console.log(existingFriendRelation1)
    console.log(existingFriendRelation2)
    if(existingFriendRelation1.length === 0  &&  existingFriendRelation2.length === 0)
      return true
    return false
  ...
  async acceptFriendRequest({auth, request}) {
    ...
    await Database.table("friends").where('user_id', RequestSender.id).where('friend_id', user.id).update("status", 1)
  ...
  async rejectFriendRequest({auth, request}) {
    ...
    await Database.table("friends").where('user_id', RequestSender.id).where('friend_id', user.id).delete()
